...
def nodesToString(nodes):
	...
	if(houver[0]==15 and houver[1]==0 or houver[0]<15):raise RuntimeError("sorry, unsupported for now")
	elif(houver[0]==15 and houver[1]==5):algtype=1
	elif(houver[0]>=16):algtype=2
...
def stringToNodes(s,hou_parent=None):
	...
	if(algtype==0):
		...
		if(hou.isUiAvailable()):
			ok=hou.ui.displayMessage("WARNING! The algorith type used by the pasted snipped is legacy and present HIGH SECURITY RISK!\n be sure you TRUST THE SOURCE of the snippet!",("CANCEL","ok"),severity=hou.severityType.Warning,close_choice=0,title="SECURITY WARNING")
		else:
			ok=0
